...
class FedAvG(FederatedModel):
    ...
    def _train_net(self,index,net,train_loader):
        ...
        # all_labels = None
        # # for batch_idx, (images, labels) in enumerate(train_loader):
        # # for batch_idx, (images, labels) in enumerate(self.testlodaers):
        # for batch_idx, (images, labels) in enumerate(train_loader):
        #     if len(images)!=1:
        #         images = images.to(self.device)
        #         labels = labels.to(self.device)
        ...
        #     # if batch_idx %10 ==0:
        # # self._classifier_weight(net,all_labels)
        #
    ...
    def _norm_evaluate(self,net,outputs,labels):
        ...
        for _, unique_label in enumerate(labels_set):
            ...
            # weight = cls_fc_weight_norm[unique_label.item()]
        ...
        print(sum_len/sum_scale)
            # print('Label:'+str(unique_label.item())+' '+'Length'+str(round(norms_mean.item(),2))+' '+'Number'+str(scale) +' '+'Weight'+str(weight))
    ...
    def _classifier_weight(self,net,labels):
        labels_set = torch.unique(labels)
        weight_dict = {}
        cls_fc_weight = net.l3.weight.detach().cpu().numpy()
        cls_fc_weight_norm = np.linalg.norm(cls_fc_weight, axis=1)
