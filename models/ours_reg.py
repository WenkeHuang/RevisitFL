...
class OursReg(FederatedModel):
    NAME = 'ours_reg'
    COMPATIBILITY = ['homogeneity']

    def __init__(self, nets_list, args, transform):
        super(OursReg, self).__init__(nets_list, args, transform)
        self.prev_nets_list = []
    ...
    def _train_net(self, index, net, train_loader):
        ...
        # cut = self.cut
        # total_communication_epoch = self.args.communication_epoch - 1
        # weight = (0.5 - cut) / 0.5 ** 3
        # target_cos = 1 - (weight * (self.epoch_index / total_communication_epoch - 0.5) ** 3 + 0.5)
        # target_cos = round(target_cos, 2)
        # target_cos = torch.tensor(target_cos).to(self.device)

        # self.prev_net = copy.deepcopy(net)
        for _ in iterator:
