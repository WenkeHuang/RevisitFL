...
class FedFeat(FederatedModel):
    NAME = 'fedfeat'
    COMPATIBILITY = ['homogeneity']
    INI = True
    def __init__(self, nets_list,args, transform):
        super(FedFeat, self).__init__(nets_list,args,transform)
    ...
    def loc_update(self,priloader_list):
        ...
        train_func = self._train_net

        if self.args.feat_regular == 1 and self.INI != True:
            train_func = self._train_with_regular

        # 部分参与更新和聚合，同样部分client才重新copyglobal
        # for i in range(self.args.parti_num):
        global_param = self.global_net.state_dict()
        for i in online_clients:
            self.nets_list[i].load_state_dict(global_param)
            train_func(i,self.nets_list[i], priloader_list[i])
        # 是否使用cl学习聚合权重
        if self.args.cl_aggregate == 1:
            ...
            # self.aggregate_nets(None)
            ws = [len(priloader_list[i].dataset)  if i in online_clients else 0 for i in range(self.args.parti_num)]
    ...
    def _train_net(self,index,net,train_loader):
        ...
        # optimizer = optim.SGD(net.parameters(), lr=self.local_lr, weight_decay=1e-5)
    ...
    def _train_with_regular(self,index,net,train_loader):
        ...
        for epoch in iterator:
            for batch_idx, (images, labels) in enumerate(train_loader):
                ...
                cosloss = self.args.cos_arg * cos(client_features,global_features,torch.ones(labels.shape[0]).to(self.device))
                loss = celoss + cosloss
                optimizer.zero_grad()
                loss.backward()
