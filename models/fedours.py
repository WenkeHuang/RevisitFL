...
#         logit_norm = torch.div(x, norms) * self.g
...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description='Federated learning via FedOurs.')
    add_management_args(parser)
    add_experiment_args(parser)
    return parser


class FedOurs(FederatedModel):
    NAME = 'fedours'
    COMPATIBILITY = ['homogeneity']

    def __init__(self, nets_list,args, transform):
        super(FedOurs, self).__init__(nets_list, args, transform)
        self.t = args.t
        self.w = args.w
        self.norm_dict = {}
    ...
    def norm_aggregate_nets(self):
        ...
        online_clients_norm_weight = [1 /(item * self.w) for item in online_clients_norm]
